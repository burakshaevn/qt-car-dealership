...
Проект соответствует архитектуре **Model-View-Controller (MVC)*:
...
2. **Запустить службу PostgreSQL** (важно!)
3. **Создать базу данных:**
   ```sql
   CREATE DATABASE car_dealership;
   ```
4. **Импортировать структуру:**
   ```bash
   psql -U postgres -d car_dealership -f schemas/car-dealership.sql
   ```
5. **Настроить параметры подключения** в `src/model/src/database_handler.cpp` (при необходимости)
...
> [!IMPORTANT]  
> После использования `windeployqt6` драйвер SQL PostgreSQL **не копируется автоматически**, что приводит к проблемам с подключением к базе данных.
...
**Что включено:**
- Все Qt DLL библиотеки
- PostgreSQL драйвер (qsqlpsql.dll)
- PostgreSQL client библиотеки (libpq.dll)
- Ресурсы приложения (изображения, иконки)
