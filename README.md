...
resources/              темы (JSON-токены + QSS), шрифт Inter, SVG-иконки, шаблон документа договора
```
...
- Дизайн-токены (цвета) описаны в `resources/themes/{light,dark}.json` и подставляются в единый
  `app.qss`; варианты виджетов задаются динамическими свойствами (`type="primary"`, `role="h1"` …).
- Монохромные SVG-иконки перекрашиваются под текущую тему, смена темы — «на лету».
